...
export async function POST(request: NextRequest) {
  return withErrorHandler(async () => {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      const project = await tx.project.create({
        data: {
          ...
          description: r.architecture?.storySummary
            || data.idea.highConcept
            || data.idea.coreConflict
            || '',
          genre: projectGenre,
          status: 'draft',
      ...
      if (r.chapters?.chapters?.length) {
        ...
        for (const vol of volumes) {
          const v = await tx.outline.create({
            data: {
              projectId: project.id, type: 'volume', order: vol.volumeNumber,
        ...
        const outlines = r.chapters.chapters.map((ch: any) => {
          ...
          return {
            projectId: project.id, type: 'chapter' as const,
        ...
        if (outlines.length > 0) await tx.outline.createMany({ data: outlines })
      }
    ...
    return apiSuccess({ projectId: created.id, project: created }, 201)
  })
}
