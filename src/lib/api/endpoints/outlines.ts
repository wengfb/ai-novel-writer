...
export const outlinesApi = {
   ...
   * 获取单个大纲节点
   */
  get: (id: string) => apiClient.get<{ outline: Outline }>(`/outlines/${id}`),

  /**
   * 创建大纲节点
   */
  create: (projectId: string, data: Omit<CreateOutlineParams, 'projectId'>) =>
    apiClient.post<{ outline: Outline }>(`/projects/${projectId}/outlines`, data),

  /**
   * 更新大纲节点
   */
  update: (id: string, data: Partial<Outline>) =>
    apiClient.put<{ outline: Outline }>(`/outlines/${id}`, data),
