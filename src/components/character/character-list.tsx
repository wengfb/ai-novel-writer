              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1 w-full max-w-full">
                    <h4 className="font-medium text-sm truncate flex-1 max-w-full">{character.name}</h4>
                    <Badge variant="secondary" className="text-xs shrink-0 text-[10px] px-1 py-0">
                      {character.role === 'protagonist' ? '主' :
                       character.role === 'antagonist' ? '反' :
                       character.role === 'supporting' ? '配' : '次'}
                    </Badge>
