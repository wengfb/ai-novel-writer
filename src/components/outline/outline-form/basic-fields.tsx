    ...
    <>
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="outline-type">节点类型 *</Label>
          <Select
            value={formData.type}
            onValueChange={(value: OutlineFormData['type']) =>
              setFormData({ ...formData, type: value })
            }
          >
            <SelectTrigger id="outline-type">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-2">
          <Label htmlFor="outline-order">排序</Label>
          <Input
            id="outline-order"
            type="number"
            min="0"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="outline-parent">父节点</Label>
          <Select
            value={formData.parentId}
            onValueChange={(value) => setFormData({ ...formData, parentId: value })}
          >
            <SelectTrigger id="outline-parent">
        ...
        <div className="space-y-2">
          <Label htmlFor="outline-target-words">目标字数</Label>
          <Input
            id="outline-target-words"
            type="number"
            min="0"
      ...
      <div className="space-y-2">
        <Label htmlFor="outline-description">描述</Label>
        <Textarea
          id="outline-description"
          value={formData.description}
          onChange={(e) => setFormData({ ...formData, description: e.target.value })}
          placeholder="描述此大纲节点的内容、关键事件与节奏..."
          rows={5}
